...
#' @export
server <- function(id, table_options) {
  moduleServer(id, function(input, output, session) {
    
    getHiddenColumns_shiny <- reactive({
      print('test')
      print(table_options[[2]])
      print('test')
      print(table_options[[1]])
      visible_columns <- getHiddenColumns_dpc_col2(table_options[[2]])
    })
    
    morphicGenesMetaDataTable <- reactive({
      #dpc_selected <- input$select_dpcs
      dpc_gene_lists <- getGeneListsFromSelect(table_options[[1]])
      #print(dpc_gene_lists)
      genesMetaDataDf_data <- subsetGenesMetaDataDf_rowsOnly(dpc_gene_lists)
    })
    
    # Can now merge with morphicGenesMetaDataTable()
    tableData <- reactive({
      dpc_selected <- input$select_dpcs
